...
static int test_gc_case(const char *name)
{
    struct test_gc_params *params;
    struct isp *isp;
    uint32_t crc;
...
static int test_gc_case_disable(void)
{
    return test_gc_case("test_gc_case_disable");
}

static int test_gc_case_enable(void)
{
    return test_gc_case("test_gc_case_enable");
}
